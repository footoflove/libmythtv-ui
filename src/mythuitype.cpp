...
void MythUIType::SetQPosition(const QPoint &pos)
{
    MythPoint xy(pos);
    if (m_Area.topLeft() == xy)
        return;
    m_DirtyRegion = m_Area.toQRect();
    m_Area.moveTopLeft(xy);
}
...
QPoint MythUIType::GetQPosition(void) const
{
    QPoint pos(m_Area.x(), m_Area.y());
    return pos;
}
...
/*frank.zhou*/
bool MythUIType::MoveTo(QPoint destXY, int msecs)
{
    if (!GetPainter()->SupportsAnimation())
        return false;
    if (destXY.x() == m_Area.x() && destXY.y() == m_Area.y())
    ...
    if(m_animation->state() != QAbstractAnimation::Stopped)
        return false;
    m_Moving = true;
    QPoint startXY(m_Area.x(), m_Area.y());
    QPoint endXY(destXY.x(), destXY.y());
    m_animation->setDuration(msecs);
    m_animation->setEasingCurve(QEasingCurve::InOutQuad);
    m_animation->setStartValue((QVariant)startXY);
    m_animation->setEndValue((QVariant)endXY);
    m_animation->start();
        return true;
}
/*~frank.zhou*/
...
void MythUIType::AddFocusableChildrenToList(QMap<int, MythUIType *> &focusList, bool is_ContainSelf)
{
    if (m_CanHaveFocus && m_Visible)
   {
	...
	if(m_Visible)
	{
	    QList<MythUIType *>::Iterator it;
	    for (it = m_ChildrenList.end()-1; it != m_ChildrenList.begin()-1; --it)
                (*it)->AddFocusableChildrenToList(focusList, true);
	}
