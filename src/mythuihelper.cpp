...
void MythUIHelper::Init(MythUIMenuCallbacks &cbs)
{
    ...
    d->maxImageCacheSize = GetMythDB()->GetNumSetting("UIImageCacheSize", 20)
                                * 1024 * 1024;
	#else
	 d->maxImageCacheSize = 20 * 1024 * 1024;
	#endif
...
{
	VERBOSE(VB_IMPORTANT, LOC + QString("LoadQtConfig function enter!\n"));
    ...
    if (themeinfo)
    {
        ...
        VERBOSE(VB_IMPORTANT, LOC + QString("Using theme base resolution of %1x%2 themename is: %3")
                .arg(d->m_baseWidth).arg(d->m_baseHeight).arg(d->m_themename));
        delete themeinfo;
    }
    ...
    //d->m_themepathname = themedir;
    VERBOSE(VB_IMPORTANT, LOC + QString("d->m_themepathname = %1\n").arg(d->m_themepathname));
	...
	VERBOSE(VB_IMPORTANT, LOC + QString("LoadQtConfig function exit!\n"));
}
...
#else
	QString tmpcachedir = cachedirname +DEFAULT_UI_THEME +
                          "." + QString::number(d->m_screenwidth) +
                          "." + QString::number(d->m_screenheight);
#endif
    return tmpcachedir;
