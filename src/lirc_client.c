			  ...
			  struct lirc_config_entry *scan)
{
	char *s;
	int do_once=1;
	//printf("lirc_execute  + 001\n");
	if(scan->flags&mode)
	{
	//printf("lirc_execute  + 002\n");
		lirc_clearmode(config);
	}
	if(scan->change_mode!=NULL)
	{
	//printf("lirc_execute  + 003\n");
		free(config->current_mode);
								   ...
								   char **string, char **prog)
{
	...
	*string=NULL;
	printf("lirc_code2char_internal code = %s\n", code);
	...
	if(sscanf(code,"%*x %x %*s %*s\n",&rep)==1)
	{
		...
		while(scan!=NULL)
		{
			//printf("lirc_code2char_internal scan + 001\n");
			...
			{
			//printf("lirc_code2char_internal scan + 002\n");
				if(exec_level > 1)
				{
					...
					//printf("lirc_code2char_internal scan + 003\n");
				}
				else
				{
					s = NULL;
					//printf("lirc_code2char_internal scan + 004\n");
				}
				if(scan->flags&quit)
				{
					quit_happened=1;
					config->next=NULL;
					scan=scan->next;
					//printf("lirc_code2char_internal scan + 005\n");
					continue;
				}
				else if(s!=NULL)
				{
					config->next=scan->next;
					//printf("lirc_code2char_internal scan + 006\n");
					break;
				}
			}
			scan=scan->next;
		...
		{
			*string=s;
			//printf("lirc_code2char_internal scan + 007\n");
			return(0);
		}
	}
