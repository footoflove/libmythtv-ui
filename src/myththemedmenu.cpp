...
void MythThemedMenu::Init(const QString &menufile)
{
    if (!m_state->m_loaded)
    {
    //VERBOSE(VB_IMPORTANT, "MythThemedMenu::Init---------------1\n");
        if (m_state->Create())
            m_foundtheme = true;
    }
    else
...
//VERBOSE(VB_IMPORTANT, "MythThemedMenu::Init---------------2\n");
    if (!m_foundtheme)
        return;
    ...
    if (!parseMenu(menufile))
        m_foundtheme = false;
	//VERBOSE(VB_IMPORTANT, "MythThemedMenu::Init---------------3\n");
}
...
{
	VERBOSE(VB_GENERAL, QString("MythThemedMenu keyPressEvent\n"));
    ...
    for (int i = 0; i < actions.size() && !handled; i++)
    {
        ...
        {
     	  VERBOSE(VB_GENERAL, QString("MythThemedMenu keyPressEvent1 %1\n").arg(action));
            bool    callbacks  = m_state->m_callback;
            bool    lastScreen = (GetMythMainWindow()->GetMainStack()
                                                     ->TotalScreens() == 2);

		 VERBOSE(VB_GENERAL, QString("MythThemedMenu keyPressEvent2 %1\n").arg(action));
            QString menuaction = "UPMENU";
            QString selExit    = "EXITING_APP";

            if (!m_allocedstate)
            {
            	  VERBOSE(VB_GENERAL, QString("Exit from thememenu0\n"));
                handleAction(menuaction);			 
            }
            else if (m_state->m_killable)
            {
            	  VERBOSE(VB_GENERAL, QString("Exit from thememenu2\n"));
                m_wantpop = true;
                if (callbacks)
                {
			...
			#if 0
            else if ((action == "EXIT" || QObject::tr("MAIN") ==
                      GetMythMainWindow()->windowTitle()) &&
                     lastScreen)
                     #else
					 else if (action == "EXIT" && lastScreen)

					#endif
            {
                VERBOSE(VB_GENERAL, QString("Exit from thememenu3\n"));
				#if 0
                if (callbacks)
                    m_state->m_callback(m_state->m_callbackdata, selExit);
                else
        ...
        {
            aboutScreen();
			VERBOSE(VB_GENERAL, QString("Exit from thememenu4\n"));
        }
        else if (action == "EJECT")
        {
            handleAction(action);
			VERBOSE(VB_GENERAL, QString("Exit from thememenu5\n"));
        }
        else
        {
            handled = false;
			VERBOSE(VB_GENERAL, QString("Exit from thememenu6\n"));
        }
		 VERBOSE(VB_GENERAL, QString("MythThemedMenu keyPressEvent3 %1\n").arg(action));
    }
...
void MythThemedMenu::customEvent(QEvent *event)
{
    if (event->type() == DialogCompletionEvent::kEventType)
    {
		...
		#else
		QString halt_cmd = "halt";
        QString reboot_cmd = "reboot";
		#endif
        if (resultid == "popmenu_exit")
        {
            switch (buttonnum)
            {
                case 0:
                    if (!halt_cmd.isEmpty())
                        myth_system(halt_cmd.toUtf8().constData());
                    break;
                case 1:
                    if (!reboot_cmd.isEmpty())
                        myth_system(reboot_cmd.toUtf8().constData());
                    break;
                case 2:
                    aboutScreen();
                    break;
...
{
	//VERBOSE(VB_IMPORTANT, QString("MythThemedMenu::parseMenu---------------1\n menuname = %1\n").arg(menuname));
    QString filename = findMenuFile(menuname);

    QDomDocument doc;
    QFile f(filename);
	//VERBOSE(VB_IMPORTANT, QString("MythThemedMenu::parseMenu---------------2\n filename=%1\n").arg(filename));
    ...
    int errorLine = 0;
    int errorColumn = 0;
	//VERBOSE(VB_IMPORTANT, "MythThemedMenu::parseMenu---------------3\n");
    if (!doc.setContent(&f, false, &errorMsg, &errorLine, &errorColumn))
    {
        ...
        if (menuname != "mainmenu.xml")
            ShowOkPopup(QObject::tr("The menu file %1 is incomplete.")
                        .arg(menuname));
		//VERBOSE(VB_IMPORTANT, "MythThemedMenu::parseMenu---------------4\n");
        return false;
    }
...
QString MythThemedMenu::findMenuFile(const QString &menuname)
{
	...
	VERBOSE(VB_IMPORTANT, QString("testdir = %1").arg(testdir));
    QFile file(testdir);
    if (file.exists())
        return testdir;
    else
        //VERBOSE(VB_FILE+VB_EXTRA, "No menu file " + testdir);
        VERBOSE(VB_IMPORTANT, "No menu file1 " + testdir);
    ...
    if (file.exists())
        ...
        //VERBOSE(VB_FILE+VB_EXTRA, "No menu file " + testdir);
         VERBOSE(VB_IMPORTANT, "No menu file2 " + testdir);
    ...
    if (file.exists())
        ...
        //VERBOSE(VB_FILE+VB_EXTRA, "No menu file " + testdir);
         VERBOSE(VB_IMPORTANT, "No menu file3 " + testdir);
    ...
    if (file.exists())
        ...
        //VERBOSE(VB_FILE+VB_EXTRA, "No menu file " + testdir);
        VERBOSE(VB_IMPORTANT, "No menu file4 " + testdir);
    ...
    if (file.exists())
        ...
        //VERBOSE(VB_FILE+VB_EXTRA, "No menu file " + testdir);
        VERBOSE(VB_IMPORTANT, "No menu file5 " + testdir);
    ...
    if (file.exists())
        ...
        //VERBOSE(VB_FILE+VB_EXTRA, "No menu file " + testdir);
		VERBOSE(VB_IMPORTANT, "No menu file6" + testdir);
...
bool MythThemedMenu::checkPinCode(const QString &password_setting)
{
	...
	#else
	QString last_time_stamp = "2011-10-10 12:11 00:00:00";
    QString password = "mythtv";
	#endif
