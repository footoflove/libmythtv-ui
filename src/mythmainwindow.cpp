...
bool MythMainWindow::screenShot(int x, int y, int x2, int y2)
{
    ...
    QString fName = QString("/%1/trid-screenshot-%2.png")
                    .arg(fPath)
...
void MythMainWindow::Init(void)
{
    ...
    QWSServer::setBackground(QBrush(QColor(Qt::transparent)));
#endif
                                       ...
                                       bool allowJumps)
{
    ...
    VERBOSE(VB_GENERAL, QString("----------------------------keynum=%1\n").arg(keynum));
            ...
            !d->jumpMap[keynum]->exittomain && d->exitmenucallback == NULL)
    {
        ...
        callback();
        VERBOSE(VB_GENERAL, "----------------------------TranslateKeyPress1\n");
        return true;
    }
        ...
        d->jumpMap.count(keynum) > 0 && d->exitmenucallback == NULL)
    {
        ...
        QCoreApplication::postEvent(
            this, new QEvent(MythEvent::kExitToMainMenuEventType));
        VERBOSE(VB_GENERAL, "----------------------------TranslateKeyPress2\n");
        return true;
    }
    ...
    if (context != "Global")
    {
        VERBOSE(VB_GENERAL, "----------------------------TranslateKeyPress2.5\n");
        d->keyContexts.value("Global")->GetMapping(keynum, actions);
    }

    VERBOSE(VB_GENERAL, "----------------------------TranslateKeyPress3\n");
    return false;
}
